...
class RVEA(BaseDecompositionEA):
    ...
    def _refresh_population(self, reevaluate_only=False):
    #def _next_gen(self):
        #Run one generation of decomposition based EA. Intended to be used by
        #next_iteration.
        if reevaluate_only:
            # Leaf GPs have changed, so refresh predictions for the same live
            # candidates.  This is model maintenance, not another evolutionary
            # generation: do not mate, select, archive, or increment FE counters.
            population = self.population
            results = population.problem.evaluate(
                np.asarray(population.individuals, dtype=float),
                self.use_surrogates,
            )
            population.objectives = results.objectives
            population.fitness = results.fitness
            population.constraint = results.constraints
            population.uncertainity = results.uncertainity
            population.ideal_fitness_val = np.amin(population.fitness, axis=0)
            population.ideal_objective_vector = (
                population.ideal_fitness_val
