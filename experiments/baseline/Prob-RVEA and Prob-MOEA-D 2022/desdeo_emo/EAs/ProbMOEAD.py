...
class ProbMOEAD_v3(MOEA_D):
    ...
    def _next_gen(self):
        ...
        for i in range(self.population_size):
            ...
            # Update the ideal point
            self._ideal_point = npmin(vstack([self._ideal_point, offspring_fx]), axis=0)
            theta_offset = float(getattr(self, "_theta_evaluation_offset", 0))
            theta_budget = float(
                getattr(
                    self,
                    "_theta_evaluation_budget",
                    self.total_function_evaluations,
                )
            )
            if theta_budget <= 0:
                raise ValueError("Adaptive theta requires a positive evaluation budget.")
            theta_progress = np.clip(
                (self._function_evaluation_count - theta_offset) / theta_budget,
                0.0,
                1.0,
            )
            theta_adaptive = theta_min + (theta_max - theta_min) * theta_progress
            # Replace individuals with a worse SF value than the offspring
            selected = self._select(current_neighborhood, offspring_fx, offspring_unc, theta_adaptive)
            self.population.replace(selected, offspring, results_off)
        self._current_gen_count += 1
