...
class Probability_wrong:
    ...
    def vect_sample_f(self):
        # Repeated calls must replace, rather than append to, the previous draw.
        self.f_samples = None
        for i in range(self.size_f):
            ...
            for j in range(self.num_objectives):
                ...
                mean = float(self.mean_values[i, j])
                scale = abs(float(self.stddev_values[i, j]))
                if not np.isfinite(mean):
                    raise ValueError("Probabilistic selection received a non-finite mean.")
                if not np.isfinite(scale) or scale <= np.finfo(float).eps:
                    scale = np.finfo(float).eps
                sample = truncnorm.rvs(-3, 3, loc=mean, scale=scale,
                                       size=self.n_samples)
