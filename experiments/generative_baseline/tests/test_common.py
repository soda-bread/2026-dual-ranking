...
class CommonTest(unittest.TestCase):
    ...
    def test_conditions_are_finite_and_in_training_range(self):
        values = np.array(
            [[0.0, 1.0], [0.25, 0.7], [0.5, 0.5], [0.7, 0.25], [1.0, 0.0]]
        )
        conditions = condition_points(values, 7, seed=3, noise=0.0)
        self.assertEqual(conditions.shape, (7, 2))
        self.assertTrue(np.all(np.isfinite(conditions)))
        self.assertTrue(np.all(conditions >= values.min(axis=0)))
        self.assertTrue(np.all(conditions <= values.max(axis=0)))

    def test_pcd_reweighting_prefers_better_fronts(self):
