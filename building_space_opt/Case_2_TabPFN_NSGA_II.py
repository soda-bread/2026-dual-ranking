# Auto-generated from Case_2_TabPFN_NSGA_II.ipynb.
# Run with: python -u Case_2_TabPFN_NSGA_II.py


# %% [markdown]
# # Case 2 TabPFN + NSGA-II
# 
# This notebook implements the building-space flexible-wall optimisation problem with a TabPFN surrogate and NSGA-II. The energy objective uses TabPFN predictions; the PMV objective remains deterministic.
...
print("Training rows:", train_df.shape, "| Test rows:", test_df.shape)
...
TABPFN_TOKEN = os.environ.get("TABPFN_TOKEN", "tabpfn_sk_oYB3VV-fcuoIOMmO48LDhBSN4lbyzmfEMV91tGJwDmA")
...
energy_predictor = TabPFNRegressor()
start = time.time()
energy_predictor.fit(X_train, y_train)
print(f"TabPFN training time: {time.time() - start:.2f} s")

if len(X_test) > 0:
    y_test_pred = np.asarray(energy_predictor.predict(X_test), dtype=float).reshape(-1)
    print(f"TabPFN test MSE: {mean_squared_error(y_test, y_test_pred):.6f}")
    print(f"TabPFN test R2:  {r2_score(y_test, y_test_pred):.6f}")
else:
    print("No held-out test rows after the training window.")
...
class EnergyCostObjective:
    ...
    def predict_energy(self, features_batch):
        ...
        if self.model_kind == "tabpfn":
            return np.asarray(self.predictor.predict(data.to_numpy(dtype=float)), dtype=float).reshape(-1)
        raise ValueError(f"Unknown model_kind: {self.model_kind}")

    def calculate_cost(self, features_batch, room_area_list):
        predictions = self.predict_energy(features_batch)
...
class FlexibleWallProblem(Problem):
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        for i, single_solution in enumerate(X):
            ...
            end_idx = len(all_features_f1)
            solution_feature_slices.append((start_idx, end_idx))
            solution_room_areas.append(room_area_list)
            result_f2.append(self.pmv_objective.calculate_pmv(features_batch_f2, room_occ_list))

        # Batch surrogate inference: one TabPFN predict call for all rooms in this pymoo evaluation batch.
        all_energy_predictions = self.energy_objective.predict_energy(all_features_f1)
        result_f1 = []
        for start_idx, end_idx in solution_feature_slices:
            room_predictions = all_energy_predictions[start_idx:end_idx]
            room_area_list = solution_room_areas[len(result_f1)]
            positive_area = np.maximum(room_area_list, 0.0)
            if positive_area.sum() <= 0:
                result_f1.append(1e6)
            else:
                weights = positive_area / positive_area.sum()
                result_f1.append(float(np.sum(room_predictions * weights)))
...
print(f"method: {METHOD_NAME} + NSGA-II")
print(f"n_gen: {N_GEN}")
